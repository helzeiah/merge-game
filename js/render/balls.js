...
function drawMergeBall(ball) {
  ...
  circleBlobPath(ball.r, ball.seed); ctx.fill();
...
function drawDyingBall(ball) {
  ...
  ctx.save();
  circleBlobPath(r, ball.seed); ctx.clip();
...
function drawDeadBall(db) {
  ...
  applyFill(_fb, r);
  circleBlobPath(r, db.seed); ctx.fill();
  ctx.save();
  ...
  const _glr = ctx.createRadialGradient(-r*0.38,-r*0.42,0,-r*0.3,-r*0.35,r*0.56);
  _glr.addColorStop(0, 'rgba(255,255,255,0.48)');
  _glr.addColorStop(0.55,'rgba(255,255,255,0.08)');
  _glr.addColorStop(1, 'rgba(255,255,255,0)');
  ctx.fillStyle = _glr; ctx.fillRect(-r, -r, r*2, r*2);
  ctx.restore();
  ctx.strokeStyle = '#111'; ctx.lineWidth = Math.max(3, r*0.13);
  circleBlobPath(r, db.seed); ctx.stroke();
